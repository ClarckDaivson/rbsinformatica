...
if (isset($_POST['btnSend'])) {
    ...
    if ((!empty($nome)) && (!empty($email)) && (!empty($telefone)) && (!empty($assunto)) && (!empty($mensagem))) {
        ...
        $email_remetente = "daivsonatanaele@gmail.com"; // deve ser uma conta de email do seu dominio
        $email_destinatario = $email; // email que receberá as mensagens
        $email_recebidoDe = "$email";
        $email_assunto = "E-mail enviado do do Site"; // Este será o assunto da mensagem
        $email_conteudo = "FORMULÁRIO DE CONTATO\n"
            ...
            . "<br><br>"
            . "<hr>"
            . "<br>Mensagem enviada do formulário de contato pelo formulario do site de cotação / contato";
